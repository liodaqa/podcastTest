...
describe('apiClient', () => {
    ...
    it('should fetch data from the correct URL', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(`${import.meta.env.VITE_API_BASE_URL || '/api'}${endpoint}`, {});
    ...
    it('should include additional fetch options when provided', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(`${import.meta.env.VITE_API_BASE_URL || '/api'}${endpoint}`, options);
        expect(result).toEqual(mockResponse);
    });
    it('should throw an error for non-OK responses', async () => {
        ...
        await expect(apiClient(endpoint)).rejects.toThrow('HTTP error! status: 404');
    });
    ...
    it('should handle cases where BASE_URL is not defined', async () => {
        ...
        const result = await apiClient(endpoint);
        expect(fetch).toHaveBeenCalledWith(`https://api.allorigins.win/get${endpoint}`, {});
        expect(result).toEqual(mockResponse);
    ...
    it('should throw an error if response JSON parsing fails', async () => {
        ...
        await expect(apiClient(endpoint)).rejects.toThrow('Failed to parse JSON response');
    });
    it('should throw an error if the endpoint is empty', async () => {
        const endpoint = '';
        await expect(apiClient(endpoint)).rejects.toThrow('Endpoint cannot be empty');
    });
});
