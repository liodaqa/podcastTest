...
import { mockCachedPodcastData, mockApiPodcastsResponse, mockCachedPodcastDetails, mockApiPodcastDetailsResponse, } from '../services/__mocks__/podcastMocks';
...
describe('podcastService', () => {
    ...
    describe('fetchPodcasts', () => {
        it('fetches and transforms podcast data successfully when cache is empty', async () => {
            ...
            expect(apiClient).toHaveBeenCalledWith('/us/rss/toppodcasts/limit=100/genre=1310/json');
            expect(result).toEqual(mockCachedPodcastData);
            expect(setCachedData).toHaveBeenCalledWith('podcasts', mockCachedPodcastData);
        });
    ...
    describe('fetchPodcastDetails', () => {
        it('fetches and transforms podcast details successfully when cache is empty', async () => {
            ...
            expect(apiClient).toHaveBeenCalledWith('/us/rss/toppodcasts/limit=100/genre=1310/json');
            expect(apiClient).toHaveBeenCalledWith('/lookup?id=1535809341&entity=podcastEpisode');
        ...
        it('handles API errors gracefully', async () => {
            getCachedData.mockReturnValue(null);
            apiClient.mockRejectedValue(new Error('API Error'));
            await expect(fetchPodcastDetails('1535809341')).rejects.toThrow('API Error');
        });
        it('handles invalid podcast data format gracefully', async () => {
            ...
            await expect(fetchPodcastDetails('1535809341')).rejects.toThrow('Invalid podcast details or data format');
        });
    });
});
