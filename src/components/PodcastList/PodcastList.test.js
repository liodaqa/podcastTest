...
describe('PodcastList Component', () => {
  it('renders nothing when the podcasts array is empty', () => {
    const { container } = render(
      _jsx(MemoryRouter, { children: _jsx(PodcastList, { podcasts: [] }) })
    );
  ...
  it('renders the correct number of PodcastItem components when podcasts are provided', () => {
    render(
      _jsx(MemoryRouter, {
        children: _jsx(PodcastList, { podcasts: mockCachedPodcastData }),
      })
    );
  ...
  it('renders nothing when podcasts prop is undefined or null', () => {
    const { container } = render(
      _jsx(MemoryRouter, {
        children: _jsx(PodcastList, { podcasts: undefined }),
      })
    );
    expect(container.firstChild).toBeNull();
    const { container: nullContainer } = render(
      _jsx(MemoryRouter, { children: _jsx(PodcastList, { podcasts: null }) })
    );
