...
describe('useFilteredPodcasts', () => {
    ...
    it('filters podcasts by name', () => {
        const { result } = renderHook(() => useFilteredPodcasts(mockPodcasts, 'Tech'));
    ...
    it('filters podcasts by artist', () => {
        const { result } = renderHook(() => useFilteredPodcasts(mockPodcasts, 'Joe Budden'));
        expect(result.current).toEqual([
            {
                id: '3',
                name: 'The Joe Budden Podcast',
    ...
    it('returns an empty array when no podcasts match the search term', () => {
        const { result } = renderHook(() => useFilteredPodcasts(mockPodcasts, 'Nonexistent'));
        expect(result.current).toEqual([]);
    });
});
