...
jest.mock('../../components/PodcastCard/PodcastCard', () =>
  jest.fn(() => _jsx('div', { 'data-testid': 'podcast-card' }))
);
jest.mock('../../components/ErrorMessage/ErrorMessage', () =>
  jest.fn(({ message }) =>
    _jsx('div', { 'data-testid': 'error-message', children: message })
  )
);
describe('PodcastEpisode Component', () => {
  ...
  it('renders an error message when the episode is not found', () => {
    ...
    render(
      _jsx(MemoryRouter, {
        initialEntries: [`/podcast/${podcastId}/episode/9999`],
        children: _jsx(Routes, {
          children: _jsx(Route, {
            path: '/podcast/:podcastId/episode/:episodeId',
            element: _jsx(PodcastEpisode, {}),
