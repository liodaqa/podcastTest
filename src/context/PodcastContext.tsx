...
export const PodcastProvider: React.FC<PodcastProviderProps> = ({
    ...
    const fetchAllPodcasts = async () => {
      try {
        ...
        const timeout = setTimeout(() => {
          if (globalLoading) {
            console.error('Fetching podcasts timed out.');
            setError('Unable to fetch podcasts. Please try again later.');
            setGlobalLoading(false);
          }
  ...
  const fetchPodcastDetail = useCallback(async (id: string) => {
    try {
      ...
      // Timeout for fetchPodcastDetails
      const timeout = setTimeout(() => {
        if (globalLoading) {
          console.error('Fetching podcast details timed out.');
          setError('Unable to fetch podcast details. Please try again later.');
          setGlobalLoading(false);
        }
      }, 10000); // Timeout after 10 seconds
